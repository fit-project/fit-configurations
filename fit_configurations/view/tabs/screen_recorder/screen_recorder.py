...
class ScreenRecorderView(TabView):
    ...
    def init_ui(self):
        ...
        # Widget
        self.enable_screen_recorder = self.find(QtWidgets.QCheckBox, "enable_screen_recorder")
        self.enable_audio_recording = self.find(QtWidgets.QCheckBox, "enable_audio_recording")
        self.audio_recording_box = self.find(QtWidgets.QFrame, "audio_recording_box")
        self.screen_recorder_filename = self.find(QtWidgets.QLineEdit, "screen_recorder_filename")
        self.verify_audio_setting = self.find(QtWidgets.QPushButton, "verify_audio_setting_button")
        ...
        # Signal
        self.enable_screen_recorder.stateChanged.connect(self._update_screen_recorder_state)
        self.enable_audio_recording.stateChanged.connect(self._on_audio_recording_changed)
    ...
    def _enable_audio_recording(self):
        if self.enable_screen_recorder.isChecked() and self.audio_setting.enable_audio_recording():
